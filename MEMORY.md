...
- **2026-07-22 — потеря данных: `Write`-перезапись untracked-файла без полного чтения.** При переводе `docs/orchestration/` на per-task T-ID схему (переворот оркестрации, см. HANDOFF.md/ARCHITECT.md) исполняющий форк сокращал `docs/orchestration/agents/bench/MIMO.md` (1462 строки), `bench/OMP.md`, `fired/AUTOHAND.md` до тонких указателей — прочитал только ~30 строк каждого и переписал файл целиком через `Write`, не проверив git-статус. Все три файла НИКОГДА не были под git (`docs/orchestration/` целиком в `.gitignore` с 2026-07-19) и, в отличие от CLINE/HERMES/GROK/ZED, не имели `archive/`-копии. Итог: история MIMO/OMP/Autohand потеряна безвозвратно — не в git log, не в trash, snapper есть только для `root`-конфига (`/home` вне снапшотов). **Правило на будущее (CLAUDE.md §Git):** перед `Write`-перезаписью любого файла целиком сначала проверять, отслежен ли он git; если нет — читать файл ПОЛНОСТЬЮ до перезаписи или не перезаписывать целиком.
...
- Critical user-accepted dark+light grim (both panels). Polish → `TBD.md`.
