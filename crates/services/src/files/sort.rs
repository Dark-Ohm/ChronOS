//! Entry sorting — ported from Chronos-FM
//! (`chronos-fm-pages/src/explorer/entries.rs`).
...
/// Sort `entries` in place: directories before non-directories, then by `key`.
/// Case-insensitive for [`SortKey::Name`].
...
/// Extension key used for [`SortKey::Type`].
