...
- **В поле сейчас (5 агентов, зоны не пересекаются):** DeepSeek №1
  доработка (`bar/widgets/network.rs` — скорость за секунду вместо
  дельты между render); Hermes №16 (`bar/widgets/tray.rs` — трей-частокол:
