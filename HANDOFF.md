...
**Обновлено: 2026-07-17, ночь-3. Волны №2-№4 закрыты кроме двух хвостов:
Autohand №3 (попап трея — доработка, живой смок противоречив) и
OpenCode №3-доработка-3 (отчёт подъехал, ещё не разобран). Читать
сверху вниз. При расхождении с ARCHITECTURE.md/
...
- **Autohand №3 — ДОРАБОТКА В ПОЛЕ (некоммичено)**: UI-попап DBusMenu
  ...
  каждую сессию).
- **OpenCode — доработка №3, отчёт ПОДЪЕХАЛ (opencode-report.md в
  корне), ЕЩЁ НЕ РАЗОБРАН.** В дереве сейчас же лежит незакоммиченный
  diff `crates/services/src/tray/menu.rs` (unwrap_variant добавлен,
  похоже на тот самый фикс лейблов) + `crates/services/examples/
  tray-menu-smoke.rs` — вероятно это его WIP/результат, разобрать вместе
  с отчётом в следующем шаге.
...
1. Разобрать OpenCode-доработку №3 (отчёт уже лежит в корне) — если
   принимается, пересобрать релиз и ПОВТОРИТЬ живой смок Autohand №3
   попапа (лейблы наконец настоящие, легче отличить «попап не открылся»
   от «открылся, но лейблы пустые»). Затем добить вердикт по Autohand
   №3 (self-destruct через ~5с — баг или моя кривая калибровка клика).
