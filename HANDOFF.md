...
Все три MIT/Apache. Статус:
  ...
  - **⏳ `gpui-rsx` вендор (Cline) — ЗАВТРА.** Бриф в `agents/CLINE.md`
    (чистая копия в `Source/gpui-rsx/`, 0 патчей — proc-macro без
    gpui-dep). Отчёт `cline-report-rsx-vendor.md` ещё не пришёл.
