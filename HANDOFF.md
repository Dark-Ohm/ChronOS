...
**2. Вендор-волна крейтов — 2 из 3 ПРИНЯТЫ (2026-07-21), rsx завтра.**
Все три MIT/Apache. Статус:
  - **✅ `gpui-animation` — ПРИНЯТ + ПРОВЕДЁН (Grok).** Вендор
    `Source/gpui-animation/` (копия `@ad77bea` + 3 форк-дельты +
    `PATCHES.md` + LICENSE-MIT/APACHE). Независимо: `cargo check` против
    форка зелёный (26с, 0 ошибок), дельты сверены построчно. Проведён в
    `Source` workspace.members + атрибуция в `Source/NOTICE`, коммит в
    Source. Отчёт: `report-log/grok-report-animation-vendor.md`. Далее
    (не начато): dep в `crates/app` + пилот hover правого кластера бара.
  - **✅ `ccf-gpui-widgets` recon — ПРИНЯТ, вендор ОТЛОЖЕН (Hermes).**
    Гейт отработал: вендор обязателен (58 ошибок, 5 классов форк-дельт,
    воспроизведено), ввод извлекаем (4 файла), тема-маппинг нетривиален
    (`Hsla→u32` + `Global`-коллизия). Отложено — не на критпути правой
    панели. Отчёт: `report-log/hermes-report-widgets-recon.md`.
  - **✅ `gpui-rsx` — ПРИНЯТ + ПРОВЕДЁН (Cline).** Вендор
    `Source/gpui-rsx/` — побайтовая копия `@307a0461`, 0 правок (чистый
    proc-macro без gpui-dep), README + LICENSE + benches. Проведён в
    workspace.members + NOTICE, `cargo check` зелёный (3.95с). Коммит
    Source `99cab5e`. Отчёт: `report-log/cline-report-rsx-vendor.md`.
    Далее (не начато): пилот на `system_popup/view.rs`.

  **ВЕНДОР-ВОЛНА ЗАКРЫТА 3/3. В поле никого.** Source ahead на 2 коммита
  (`66cd816` animation, `99cab5e` rsx) — вендор-крейты готовы к
  использованию (dep из `crates/app` — отдельная задача). Следующее
  крупное поле — **капстоун: правая боковая панель** (план готов, код
  нулевой, см. п.1).
  - **Мусор:** `reports/opencode-report-4.md` — дубль уже принятого
    `cbfc197` examples-recon, всплыл повторно. Удалён из активных reports.
