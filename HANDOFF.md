...
**Обновлено: 2026-07-18. Autohand СНЯТ С ПРОЕКТА (решение
пользователя) — его трек (tray_menu, задание №3) передан Hermes,
задание №10 в HERMES.md. Cline №7 и Hermes/Autohand №9 (bugfix-пара)
ПРИНЯТЫ. Системный баг `remove_window()` теперь разобран ПОЛНОСТЬЮ —
две причины, обе с фиксом/заданием (см. раздел ниже — читать
ОБЯЗАТЕЛЬНО, если трогаешь окна). Волна №5: Grok №5 (MPRIS) ПРИНЯТ,
Grok №6 (Source flush-фикс) ПРИНЯТ частично, Mimo №6 (dock) НЕ
ПРИНЯТ — критичный баг + инцидент с чужими строками в коммите.
Открытые хвосты: Cline №8 (launcher реентерабельный close), Hermes
№10 (tray_menu, унаследовано от Autohand). Читать сверху вниз. При
расхождении с ARCHITECTURE.md/DECISIONS.log побеждают они.**
...
3. **Grok №6 ✅ ПРИНЯТ ЧАСТИЧНО** (Source `3800d3a`, sync flush+unregister
   ...
   Роздано дальше: **Cline №8** (launcher, готовый диагноз в CLINE.md).
