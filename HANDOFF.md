...
**Не разобрано до конца.** Кандидат для отдельного расследования —
не чинить симптомы по одному в каждом модуле (soft-hide работает как
обход, но это костыль на каждый конкретный случай), а понять корень в
`Source/gpui_linux/src/linux/wayland/window.rs::resize`/remove path.
Если наткнёшься на «window not found» или окно не закрывается —
это, вероятно, ОНО, а не баг твоего кода — сверься с этим разделом
прежде чем начинать чинить с нуля.
...
7. Расследовать системный `remove_window()` баг (см. новый раздел выше) —
   отдельная сессия, не затычки по модулям.
