...
**Обновлено: 2026-07-29 (вечер) — T156 закрыта, T150/T152/T154 приняты с
эрратами, T155 поглощена, база замера сдвинулась, Hindsight переехал на
compose. Ниже — актуальное; всё, что дальше вниз, — история.**

### Где стоим (2026-07-29, вечер)
...
- **`LICENSE-APACHE` неприкосновенен.** В T155 его снесли вместе с
  ...
  сохранять copyright notice. Урок — в `ARCHITECT.md` (`68c671f`).
...
`MEMORY.md`, попытки архитектора кликнуть синтетически дали
...
`design/Updates Popup.dc.html` (dark+light). Побочная находка при
планировании: `roadmap.md` ошибочно считал светлую тему непорченной —
...
`design/System Sidebar.dc.html` и решил: **пересобрать панель пиксель-в-пиксель
...
- **Разведка компиляцией роздана (2026-07-20):** Cline №2 — `gpui-rsx`
  ...
  gpui; конфликт со STYLE.md только через `class="bg-*"` — решение:
...
- **`gpui-form` разведка — ЗАКРЫТА, ПРИНЯТА.** Zed не смог (сломанный
  ...
  `roadmap.md` как открытый путь, не задача.
...
- **ВОЛНА «разведка форка» роздана 2026-07-20 (4 агента, READ-ONLY по
  ...
  - **`9c2c090` DeepSeek №2 (бывшее №14) — ПРИНЯТ. Волна разведки форка
    ...
    из Kael, которые DECISIONS.log 2026-07-16 планировал портировать
...
- **Вывод апгрейда (живой репорт 2026-07-20):** «Upgrade all» уходит в
  ...
  см. кровные факты.)
  - Бриф Claude Design написан (`design.md`, `472f99b`) — состояния
