...
**Очередь:** T147 (потолок хода — эррата к T146/E2: константа зовётся
абсолютной, но проверяется только по тишине) → T144 (список моделей; бамп на
2.0.0 его НЕ починил — `NewSessionResponse` больше не несёт `models` вовсе).
