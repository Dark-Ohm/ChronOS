...
Theme-токены. Сейчас — хексы прямо из мокапа.
  - **Трек 1 (в поле):** Hermes №22 — rsx-визуал пиксель-в-пиксель. Данные: метры/
    сеть/часы/транспорт живые; арт/прогресс/диски/пермишн/заголовок — статик.
  - **Трек 2 (бэкенд, очередь):** udisks2-сервис — секция дисков **вместо
    батареи**: полоса-usage на каждый диск/USB; внутренние — только полоса,
    съёмные — + mount/unmount/безопасно-извлечь.
  - **Трек 3 (бэкенд, очередь):** расширение MPRIS (`artUrl`/`Position`/`length`)
    → **обложка альбома** в кадр 16:9 (НЕ видео — видео будущий режим) + прогресс/
    таймкод.
