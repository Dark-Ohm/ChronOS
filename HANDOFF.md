...
- **Роздано 2026-07-20 (добить бар, три параллельных брифа, зоны не
  пересекаются):** Grok №15 — свип 7 попапов на палитру STYLE.md
  (`bg.primary` база); Mimo №11 — battery/mpris с эмодзи на SVG
  (последние эмодзи бара) + hover-свип 6 кликабельных виджетов права
  + CAVA 2.5/16; GLM №1 (НОВЫЙ агент, GLM.md) — светлая
  тема Light C в `light_scheme()` + выбор схемы через `CHRONOS_THEME`
  (механизма выбора схем до этого не существовало — `Theme::init`
  ставил только дефолт); DeepSeek №1 (НОВЫЙ агент, DEEPSEEK.md) —
  редизайн network-виджета: лампочка активности (серая/зелёная по
  трафику, error при отвале сети) + спидометр двумя строками ↓/↑
  (procfs-счётчики на 1s-тикере, прецедент — парс `.git/HEAD`), глиф и
  SSID выпиливаются, виджет остаётся некликабельным.
