...
**Очередь:** **T158** (роздана, бриф написан по итогам замера) → **T159** →
**T160** → **T161** → **T162**. Слайс 1 лежит в `active/`, но помечен в
файлах ролей как «в очереди, НЕ начинать без команды архитектора».

**Премисса T158 под сомнением, и это записано в бриф.** При `lto = true` +
`strip = true` линкер уже выбросил всё неслинкованное — значит +1.96 MiB
это УЖЕ обрезанная цифра, и резать 77 модулей исходника компонента может
дать ноль. Часть 1 задачи — дешёвый эксперимент на опровержение: дельта
< 50 KiB закрывает вопрос обрезки навсегда. Часть 2 — `Root` и
`KeyboardInteractivity::OnDemand` из подпорок замера в постоянную проводку,
причём `OnDemand` обязан быть проверен против §7 спеки (правая панель не
закрывается по фокус-лоссу) — там единственное место, где может вскрыться
настоящая проблема.
...
бара из `STYLE.md`, 144 FPS — всё подтвердилось грепами. Четыре решения,
...
реверс июльского «варианта C» (`DECISIONS.log`, `ee63c19`). Цена входа за
...
(`DECISIONS.log` 29.07, коммит `ee63c19`). Реверс июльского «варианта C»
