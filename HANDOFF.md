...
ПРИНЯТ (build+177 тестов зелёные, изолированный worktree). НО живой
смок после приёмки вскрыл ДВА свежих репорта пользователя, оба
задокументированы и розданы: Cline №9 — лаунчер закрывается от
голого движения мыши (корень: `follow_mouse=1` в hyprland.lua гонит
клавиатурный фокус за курсором, наблюдатель активации видит это как
настоящий blur — НЕ регресс №8, отдельный кейс); Hermes №11 — попап
уведомлений снова обрезается снизу (эвристика `estimate_content_height`
не учитывает перенос строк в summary). Автономный `follow_mouse`-баг
объясняет и мой предыдущий неоднозначный ручной смок №8 (иногда
закрывалось само за доли секунды без явного триггера — это оно).
Системный баг `remove_window()` (реентерабельность) разобран
