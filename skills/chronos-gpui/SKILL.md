...
| Does the fork have CSS grid? | **Yes** — `.grid()`, `.grid_cols(N)`, `.grid_rows(N)`, `col_start/end/span`, `row_*` (Taffy-based). Live users: `examples/grid_layout.rs`, `examples/anchor.rs`. T231 used it for 2-col responsive settings grids. Grep `../Source/gpui` too — the fork is *outside* the repo root. | `styled.rs` grid block, `examples/grid_layout.rs`, T231 2026-08-04 |
