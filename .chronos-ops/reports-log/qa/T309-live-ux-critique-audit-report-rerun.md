...
# ПРИЁМКА АРХИТЕКТОРА — ПОВТОРНЫЙ ПРОГОН, ПРИНЯТ ЧАСТИЧНО (2026-08-19)

T309 как тикет закрыт 2026-08-19 первым прогоном (`done/qa/`, отчёт в
`reports-log/qa/`). Это повторный аудит по тому же брифу. Разобран
отдельно.

**Итог: одна находка проверена и ценна, остальное — либо уже известно,
либо уже починено, либо третий подряд повтор одной методологической
ошибки.**

## ПРИНЯТО — WCAG на приглушённом тексте

Единственная находка, которую стоило принести. Пересчитал сам, из
палитры в дереве (`crates/ui/src/theme/schemes.rs`), формула WCAG 2.x
по относительной яркости:

```
dark  muted   #6c7086 на bg.primary  #1e1e2e → 3.36:1
dark  muted   #6c7086 на bg.tertiary #181825 → 3.59:1
light muted   #7d80a6 на bg.primary  #dde0f2 → 2.91:1
light muted   #7d80a6 на bg.tertiary #eceefa → 3.30:1
```

Цифры отчёта совпали до сотых. `text.muted` в светлой теме не проходит
даже порог для крупного текста (3.0), в тёмной — на грани. Заведён
**T317**.

Для контекста: `text.secondary` проходит в обеих темах (7.37 и 4.84),
так что дефект локализован в одной роли, а не в палитре целиком.

## ПРИНЯТО, НО НЕ НОВОЕ — dock и pinned apps

`dock: skipping pinned app ... no matching .desktop basename` для
firefox/code/vivaldi — та же находка, что уже принята и записана в
`MIGRATION.md` по первому прогону T309. Повтор, не новая информация.

## ОТКЛОНЕНО — «критическое №1»: прозрачная правая панель

Отчёт ставит это первым номером и называет «точно НЕ design intent».
Это design intent, и он разбирался и закрывался уже дважды.

Замерена **техническая канва**, а не панель. T276: `content` — окно
фиксированной ширины `CONTENT_CANVAS_WIDTH = 920`
(`side_panel_right/mod.rs:70`), которое НИКОГДА не ресайзится; красится
только видимый прямоугольник внутри неё, ширина берётся из
`visible_content_width`. Остаток канвы прозрачен по конструкции и
кликов не ест — `content_input_region(0.0, _, false).is_empty()`,
проверено в T302.

Состояние «панель открыта, вкладка не выбрана» — rail-only по T220,
разобрано и закрыто в **T302** («бага нет, by-design»), причина там же:
призыв «сразу с чатом» прятал композер (баг T137).

Полностью прозрачное окно неотличимо от его отсутствия — видеть
«стекло» и «дыру» не в чем. Мой промер на белых обоях с обеими
открытыми панелями: строка y=700, `x=56..2503` сплошной
`(255,255,255)` — панель не рисует ничего, границы нет.

**Таблица по одиннадцати вкладкам не может быть настоящей.** У каждой
вкладки своя `preferred_content_width` (`side_panel_right/tabs.rs:773`):
System 400, Updates 420, Files и SourceControl 440, Editor/Terminal/
Preview 560, Build 640. При этом отчёт даёт:

- Files (440) → 47/46, SourceControl (440) → 51/40 — **одинаковая
  ширина, разные числа**;
- Build (640), Editor (560), SourceControl (440), Library, Scenes,
  Captures, Inspector — **все семь дают ровно 51/40**, хотя ширины
  различаются в полтора раза.

Девять одинаковых значений подряд означают, что переключения вкладок
не происходило и один и тот же кадр измерен девять раз. Числа не
свидетельствуют ни о чём.

## ОТКЛОНЕНО — «24-пиксельная щель»

Щель была 16px, не 24. Промер живого шелла до фикса: плита `0..15`,
обои `16..31`, рельс `32..70`. Аннотация отчёта сама себе противоречит:
щель заявлена как `16..40` (то есть 24px), а рельс как `32..72` — эти
два диапазона перекрываются.

Причина найдена и устранена до разбора этого отчёта: двойная
компенсация края (эксклюзивная зона `frame_wrap_excl_left` 16px плюс
`margin.left = wrap_inset()` 16px у рельса). Коммит `fdb69638`. Отчёт
снят на бинаре 08:27, то есть до фикса — претензий к исполнителю нет,
но находка неактуальна.

## ОТКЛОНЕНО — §14 «wrap-режим должен быть визуально невидимым»

Третий заход одной и той же ошибки. Вывод строится на сэмпле мокапа
`x=0..40`, который целиком лежит внутри левой служебной зоны шириной
60px и потому однороден по определению. Однородность доказывает
обратное тому, что из неё выводят.

Перепромер мокапа (`ffmpeg -ss 12` и `-ss 40`, 1920×1080): плита есть
по всему периметру, `#FCF5F3` в светлой и `#121116` в тёмной; инсеты
60 слева (там рельс) и по 10 сверху/справа/снизу; внутри прорезана
скруглённая апертура. Рельс в мокапе — не отдельная поверхность, а
левый край той же плиты, расширенный под иконки.

Полный разбор — `.chronos-ops/active/design/T310-*.md`, раздел A
дополнения. Предложение «не рисовать плиту» отменено там же: оно даёт
ровно противоположную мокапу картинку.

## ОТКЛОНЕНО — §15 «Mocha Mousse вместо Catppuccin»

`#FCF5F3` из мокапа — тёпло-белый. Pantone 17-1230 Mocha Mousse —
коричневый `#A47864`. Это разные цвета; модное имя приклеено к чужому
фону. Тёмная половина мокапа вообще холодная (`#121116`) с розовым и
фиолетовым акцентами — тёплой палитрой там не пахнет.

Решение владельца: палитра ChronOS остаётся своя, Mocha Mousse
добавляется как обычная схема наравне с Solarized, плюс заводится
theme picker. Нарезано в **T313**, приоритет P3, дефолт `theme.toml`
не меняется.

## ЭРРАТА — `workspace_mode`

Отчёт: «в `bar.toml` нет такого виджета». Наоборот — он там есть,
`~/.config/chronos/bar.toml:37`. Его убрали из `BUILTIN_NAMES` в T292,
пилюля переехала на правый рельс (`layout_config.rs:618-621`,
тесты `:685`, `:892`). Неизвестное имя виджета молча отбрасывается —
это документированный инвариант конфига.

Эффект отчёт описал верно, причину — наоборот. Реальный остаток:
у владельца в конфиге висит протухшая запись, которая тихо
игнорируется. В `checkpoint/TBD.md`, тикета не стоит.

## Методологический урок

Три находки из этого отчёта построены на замере технической
ограничивающей рамки и объявлении прозрачного остатка дефектом:
канва правой панели, сэмпл мокапа, гэп рамки. Один и тот же приём.

Правило на будущее для QA-прогонов: **прежде чем называть прозрачную
область дефектом, ответить, кто в этой области обязан был рисовать.**
Если ответа нет — это не дыра, это отсутствие объекта. И числа,
одинаковые для сущностей с разными параметрами, — сигнал, что
эксперимент не менялся, а не что система однородна.

## Что сделано по итогам

- **T317** — WCAG на `text.muted`, заведён в `active/front/`.
- Остальное закрыто ссылками на T302, T310 (раздел A), T313, `fdb69638`.
- Отчёт уходит в `reports-log/qa/` как повторный прогон.
