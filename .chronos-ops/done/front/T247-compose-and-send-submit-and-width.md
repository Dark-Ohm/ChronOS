...
**ЗАКРЫТ.** Корень найден живым прогоном: `send_composer` молча дропал
сообщение при `agent_status == Thinking` — compose-and-send приходил за
4s до коннекта ACP-клиента. Фикс: очередь `pending_send` только в фазе
подключения (юзер-сообщение пушится сразу + честная заметка, дренится на
коннекте), mid-turn — честная заметка без очереди (иначе застряла бы),
connect-фейл/switch/новая сессия чистят очередь. Width-desync не
воспроизводится (T243-зеркало уже чинит — 352px держится). Живая
верификация: очередь → авто-сабмит → реальный ответ агента; mid-turn не
спавнит параллельный ход; тесты 13/13, сборка чистая. Отчёт:
`docs/orchestration/tasks/report-log/T247-compose-and-send-submit-and-width-report.md`.
