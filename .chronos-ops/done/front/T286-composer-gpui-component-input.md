...
**Статус:** ГОТОВА К ВЫДАЧЕ (2026-08-16). Ранее "не параллелить с T285" —
T285 закрылся STOP-ом апстрима БЕЗ единого изменения кода (`chat.rs`/
`composer.rs` не тронуты), так что зонного конфликта, ради которого
держали guard, больше нет. Функционально T286 с T285 не связана —
"не трогать ACP connect/create_session/load_session" ниже остаётся в
силе как есть (это чужая логика, не зависимость от её судьбы).
...
`chat.rs` / `composer.rs` — не параллелить с T285. T287-C и T287-A не
начинать, пока T286 не в git (Follow и пикеры живут в том же
`composer.rs`). Пикеры model/mode здесь не трогать.
...
2026-08-16, живой смок Neo подтвердил: перенос строк внутри карточки
(длинная строка без пробелов и с пробелами) и Ctrl+Enter — новая строка
(не Shift+Enter, как в спеке выше — поправка по факту). Enter — один
send, compose-and-send:<text> — один turn. Коммит `c5151fb9`. Принято.
