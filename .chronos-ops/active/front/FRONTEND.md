# FRONTEND — точка входа роли (кухня `.chronos-ops`)

**Роль:** UI, взаимодействие, тема — ChronOS. Не пишет сервисы/IPC/
packaging (это BACKEND).

**Общие правила:** `.chronos-ops/RULES.md` — прочитать перед стартом.

## Очередь

1. **T322** — `T322-panel-inset-live-instead-of-recreate.md`. P1,
   СВОБОДЕН. Панели пересоздают поверхности при смене стиля рамки —
   каскад роняет адаптер, шелл теряет ВСЕ слои до рестарта. Остаток
   T321: тот же приём «мутировать живьём вместо close+open».
2. **T316** — `T316-bar-radius-closes-aperture-top.md`. P2, СВОБОДЕН
   (T318 принят, радиус = 10). Нижние углы бара замыкают апертуру
   сверху. Остаток T311 D4.
3. **T312** — `T312-frame-modes-normal-wrapped.md`. P2, СВОБОДЕН.
   Два режима `normal`/`wrapped`, алиасы старых имён в
   `deserialize_style`.
4. **T313** — `T313-theme-picker-and-mocha-mousse-scheme.md`. P2,
   СВОБОДЕН. Picker схем со свотчами + схема Mocha Mousse.

T322, T316 и T312 параллелить нельзя — все трогают раму. Порядок
T322 → T316 → T312.

T313 независим от рамы (зоны `crates/ui/src/theme/`,
`side_panel_right/tab/bar_settings.rs`, `theme_config.rs`) — идёт
параллельно с чем угодно. T317 закрыт — его WCAG-тест итерирует
`builtin_schemes()`, поэтому схема Mocha Mousse обязана пройти те же
ворота автоматически.

**Кольцо апертуры (`aperture_ring`, `frame.rs`) переделке не подлежит** —
форма принята владельцем 2026-08-19 после трёх неверных заходов. Прежде
чем трогать скругления оболочки, прочитать разбор в
`reports-log/front/T318-rail-as-frame-edge-implementation-report.md`:
там расписано, почему `rounded_*` на рельсе и угловые заплатки дают
вывернутую кривизну, а работает только кольцо с бордером.

**Закрыто 2026-08-18/19 (детали — `MIGRATION.md`):** T301, T302, T303,
T305, T307, T308, T311 (единая плита), T314 (живая эксклюзивная зона —
рельс стал кромкой кадра), T318 (оболочка обводит окно), T317 (WCAG muted), T319 (геометрия по краям), T321 (живая геометрия
без рекрейта), T320 (вкладки
вернулись в панель, control-center снят).
