...
- Композиция бара из `docs/STYLE.md`: CAVA строго по центру, часы — крайние справа. Новый виджет встаёт **только** в правый кластер и **только** левее `clock`; трогать `center` запрещено.
...
//!
//! Живёт СТРОГО в правом кластере левее часов: `docs/STYLE.md` фиксирует CAVA по
...
Написать отчёт в `docs/orchestration/tasks/report/<T-ID>-workspace-mode-slice-1-report.md`, где `<T-ID>` — номер, выданный архитектором в задании (он же в заголовке твоего `docs/orchestration/tasks/active/`-файла). Отчёт содержит точный перечень изменённых файлов, выводом тестов и честным списком того, что НЕ проверено живьём. Не заявлять «работает», если шелл не запускался.
