...
- **2026-08-16, уточнение:** скелет `.chronos-ops/` коммитим (иначе
  ...
  по одному тикету.
- **2026-08-17, чекпоинт #36 — снятие `.chronos-ops` с git-tracking.**
  **Рассмотрено:** кухня как локальное дисковое состояние, не
  git-контент (по образцу экосистемного `.chronos-ops`, который вне git
  осознанно — корень экосистемы не репозиторий). Пробовал: `git rm
  --cached` + `.gitignore` на весь `.chronos-ops/`. **Отклонено в тот же
  день:** «контрибьюторы должны откуда-то брать задания» — кухня ЭТОГО
  репо, в отличие от экосистемной, это первичный источник заданий для
  людей, клонирующих репо, а не рабочий кэш архитектора. Откат:
  `.chronos-ops` возвращён под git тем же коммитом. Правило на будущее:
  разница «кухня репо» vs «кухня экосистемы» не в git/не-git по аналогии
  — она в том, кто клиент кухни (контрибьютор конкретного репо vs
  кросс-репо координация без единого чекаута).

## 2026-08-17 — T285: `attach_session`/`ActiveSession` из публичного API

- **Рассмотрено:** починить cold `load_session`, чтобы он сажал
  `ActiveSession`, а не создавал новую сессию через `create_session`.
- **Отклонено — STOP, не архитектурный выбор, а тупик апстрима.**
  `attach_session`/`ActiveSession` в `agent-client-protocol` **2.0.0**
  приватные — регресс относительно `0.11.1`, где они были публичными.
  Подтверждено сверкой с исходником крейта в `~/.cargo/registry`.
  Фикс на нашей стороне невозможен без форка/патч-бампа крейта или
  smyков в v2-wire. Гейт 8 (live restart-restore) провален живьём
  2026-08-16 — не гипотеза, воспроизведено. Рекомендация: upstream issue
  в `agent-client-protocol`, не блокирует остальной T286/T287/T287-C
  (файловый guard, который держал их, оказался нефункциональным и снят
  отдельно). Ветка `feat/t285-acp-load-session` держится в git как
  парковка на случай апстрим-фикса, воркетри снесён (2026-08-17,
  зачистка воркетри).

  ## 2026-08-22 — T352: swaybg per-monitor Set теряет картинку на другом мониторе, не чиним

  - Рассмотрено: `apply_swaybg` (`crates/services/src/wallpaper/backends.rs`,
    T349) собирает ОДИН `swaybg_argv` под текущую команду и не портирует
    waytrogen'овский `SWAYBG_WALLPAPERS`-реестр (`changers/swaybg.rs`,
    T348-отчёт), который держит per-monitor состояние и пересобирает ВСЕ
    известные `-o/-i/-m/-c`-блоки в один процесс при каждом Set. Живой
    прогон T352 подтвердил регресс: Set на мониторе A, затем Set на
    мониторе B — restart (`pkill -9 swaybg` + новый процесс) убивает
    картинку на A, ChronOS-версия её не восстанавливает.
  - Решено: **принять регресс, не чинить.** Per-monitor Set у swaybg
    сейчас физически недостижим ни из одного UI/IPC-пути ChronOS —
    `wallpaper_ctl::next()` и IPC `wallpaper-set` всегда шлют команду без
    monitor, диспетчер `dispatch()` не получает per-monitor команду ни
    от кого. Чинить нечего, что не используется. Портировать реестр —
    когда T350 (галерея) добавит monitor picker и per-monitor Set станет
    реальным юзкейсом, не раньше — заводить тикет на непроверенное
    предположение сейчас смысла нет.
  - На «All» (единственный реально используемый режим) регресс не
    проявляется вовсе — один процесс без `-o`, реестр не участвует.
