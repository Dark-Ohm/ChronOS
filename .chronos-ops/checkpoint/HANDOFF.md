...
`checkpoint/log/handoff.log` (~4000 строк, грепать по T-ID/дате/симптому,
не читать подряд; разрезка на несколько файлов — тикет T306).
...
FRONTEND, очередь: **T308** (P0, регресс на T303) → **T307**.

**T308 (P0, живая находка владельца, 2026-08-18 вечер):** с открытой
боковой панелью wrap-матте съезжает на 56px (40 резервация рейла + 16
wrap-инсет) — правое кольцо улетает за экран (2560+56=2616), левое почти
целиком скрыто под самим рейлом. Найдено по скрину владельца, подтверждено
`hyprctl layers -j` на его живом шелле: `frame_wrap_matte x=56` вместо `0`.
Подозреваемый корень: матте `exclusive_zone: None` — не opt-out
`Some(px(-1.))` как у T305 control-center popup, та же ловушка. T303
остаётся принятым **только для случая без открытых боковых панелей** —
это не отменяет T303, но его живой grim не покрывал этот сценарий.

**Волна 2026-08-18 принята:** T305 (`f326fc7`, control-center popup —
кровный факт `exclusive_zone: Some(px(-1.))`), T303 (`c6df21a`, wrap
matte-геометрия — `LEFT|BOTTOM` + отрицательный `margin`), T301
(`96f713a`, Select-попап эллипсис — `max_w` вместо `w_full`, живой грим
снят архитектором после протухшего `ydotoold`). Детали — `MIGRATION.md`
и `active/front/FRONTEND.md` («Закрыто 2026-08-18»). Находка T303 не в
зоне (wrap-геометрия не hot-reload'ится на живом шелле) вынесена в
**T307**, единственный пункт очереди.
...
| `checkpoint/log/handoff.log` | журнал сессий (архив) |
