...
**T305 принят 2026-08-18** (`f326fc7`) — settings-табы правого рейла +
Media переехали в единый anchored control-center popup у иконки рейла.
Кровный факт: `exclusive_zone: Some(px(-1.))` — opt-out от резервации
рейла, иначе композитор сдвигает popup на 40px+бар. Решение №1 брифа
пересмотрено владельцем: иконки остаются на рейле, `ALL`/`for_mode` без
изменений (21 таб, Media popup-only). Живой смок на release снят.

**T303 принят 2026-08-18** (`c6df21a`) — `wrap.thickness` разведён от
`bottom_strip.height`, debug-лог выпилен. Кровный факт: matte —
`LEFT|BOTTOM`-якорь с **отрицательным** `margin` компенсирует резервации
ровно (`bottom=-inset`, `left=-inset`); `LEFT|RIGHT` центрирует
переполнение при открытой панели; слой `Top` не `Overlay` (тот глушит
клики рейла/панелей); `border_t_0()` — верх рисует бар. Находка вне зоны:
wrap-геометрия не hot-reload'ится на живом шелле (`apply_wrap`
early-return при открытой матте) — вынесена в T307.

**Очередь FRONTEND (порядок жёсткий):**

1. **T301** — P3, хвост T298: текст в composer Select-попапе без
   эллипсиса.
2. **T307** — P3, находка T303: wrap-геометрия (`thickness`/
   `inner_radius`) не подхватывается на живом шелле без рестарта.
