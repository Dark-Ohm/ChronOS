...
Очереди `back`, `recon`, `qa`, `design`, **`front`** пусты. Ждём новую
работу от владельца.
...
(`f2cacee`, wrap-матте `exclusive_zone: Some(px(-1.))` — без явного
opt-out Hyprland смещал anchor-only матте на сумму ВСЕХ чужих резерваций
(рейл 40 + свои 16 = 56px), с `-1` резервации игнорируются целиком и
отрицательный margin T303 стал не нужен (двойная компенсация уводила в
минус) — снят). Матте теперь `x=0,y=0` при любой комбинации открытых
панелей, проверено живьём (закрыты/левая/правая/обе). Детали —
`MIGRATION.md` и `active/front/FRONTEND.md`.
