...
FRONTEND, очередь: **T308** (единственный тикет, P0).
...
целиком скрыто под самим рейлом. `hyprctl layers -j`: `frame_wrap_matte
x=56` вместо `0`. Подозреваемый корень: матте `exclusive_zone: None` — не
opt-out `Some(px(-1.))` как у T305 control-center popup, та же ловушка.
**Независимо подтверждено T307-исполнителем** (тот же сдвиг, идентичен в
cold-старте и после hot-reload — не регресс T307, зона T303). T303
остаётся принятым только для случая без открытых боковых панелей.

**Волна 2026-08-18 принята:** T305 (`f326fc7`), T303 (`c6df21a`), T301
(`96f713a`), T307 (`601f8f0`, wrap thickness/inner_radius теперь
hot-reload'ится — живой смок сам: `w:16→8` на лету, лог «hot-reloaded
config», эквивалентно cold-старту). Детали — `MIGRATION.md` и
`active/front/FRONTEND.md`.
