...
- Maintain `.chronos-ops/checkpoint/HANDOFF.md` (**state only, keep under
  ~200 lines — anything stale moves to `checkpoint/log/`**),
...
### Инструмент подозревается раньше кода (added 2026-08-13, T272)

Четыре улики за один день: сборка исполнителя упала `exit 101` на
doc-only правке (общий `target/` между деревом и воркетри), два мёртвых
воркетри прожили лишние дни, а дерево T266 было создано от базы БЕЗ
коммита-зависимости T263.

Правила:

1. **База воркетри доказывается.** `git merge-base --is-ancestor <dep>
   HEAD` перед стартом. Тикет зависит от другого — коммит-база названа в
   задании явно.
2. **Свой `target/` на воркетри.** Не делить между деревьями, которые
   собираются параллельно. Перед разбором ЛЮБОГО непонятного отказа
   сборки: `echo $CARGO_TARGET_DIR`, `ls -d ../ChronOS-wt-*`.
3. **Окружение подозревается раньше работы.** Необъяснимый `exit`,
   таймаут, «несамодостаточный коммит» — сперва target/воркетри/ядро/
   allowlist, потом обвинение исполнителя. Прецедент: я однажды почти
   обвинил исполнителя зря именно на общем `CARGO_TARGET_DIR`.
4. **Воркетри умирает вместе с задачей** — сносится тем же действием,
   что переносит тикет в `done/`. Долгоживущее исключение — только с
   записанной причиной.

### Тавтологический тест и цена нетестируемого редьюсера (added 2026-08-14, T278)

Round 3 T278 пришёл с тестом `on_dock_toggle_uses_pure_helper`, который
не вызывал `on_dock_toggle`. Тело: присвоить результат хелпера в глобал,
затем проверить, что глобал равен результату хелпера. Зелёный при любом
состоянии продакшн-функции — хоть удали её целиком. Имя и докстрока при
этом заявляли покрытие интеграционного пути.

Отдельно — оправдание в докстроке: «интеграционный путь покрыт
продакшн-кодом». Продакшн-код не является тестом; такая формулировка
есть признак того, что тест писался под зелёный, а не под доказательство.

Правила:

1. **Тест обязан звать проверяемый путь по имени.** Нет вызова —
   переименовать в честное или удалить. Строка «юнитом не покрыто,
   проверяется живьём в TNNN» принимается; тавтология под именем
   интеграционного теста — нет.
2. **Нетестируемый редьюсер — дефект архитектуры, а не повод не
   тестировать.** Здесь entity не поднималась в `TestAppContext`, потому
   что `SidePanelLeft::new` спавнит async ACP-connect. Верный вывод
   (round 4): редьюсер становится свободной функцией на `&mut App`
   (`apply_dock_toggle`), вьюха делегирует в одну строку. Если состояние
   нельзя изменить без живого продуктового дерева — это метод вьюхи, а
   не редьюсер.
3. **Ветки теста доказывают чтение состояния.** Ветка с
   `remembered.chat = 700` и ветка с фиксированной вкладкой отличают
   «прод читает состояние» от «прод совпал со статической константой».
   Без них зелёный ничего не значит.
4. **Сверять счёт тестов с выводом команды.** Расхождение 71/72 здесь
...
### Блокер миньона — гипотеза, а не факт (added 2026-08-21, T329)

Исполнитель T329 сдал корректный код с приписанным ему «блокером»:
календарь якобы не открывается ни с правкой, ни на чистом HEAD, вина
на canvas-обёртке бара, гипотезы для владельца, долг на отдельный
тикет. Приёмка живьём: **попап открывается с первого клика тем же
бинарником**. Оба «доказательства» блокера были методическим браком:

- `ydotool mousemove --absolute` на этой машине НЕ попадает в
  заказанные координаты (акселерация виртуального устройства):
  `-x 2404 -y 10` → фактически `3475, 39`, т.е. второй монитор,
  `-x 2000 -y 1000` → `4479, 1131` (упор в правый край). Исполнитель
  весь день кликал в пустоту и строил из этого архитектуру. Warping —
  только `hyprctl dispatch 'hl.dsp.cursor.move({x=..,y=..})'`
  (Lua-Hyprland 0.56.2; старый `movecursor 2404 10` — Lua-ошибка), и
  позицию сверять `hyprctl cursorpos` ПЕРЕД кликом.
- Отсутствие попапа в `hyprctl layers` было принято за «окно не
  создано». Якорный попап — xdg_popup-ребёнок родительского
  layer-surface: его нет ни в `layers`, ни в `clients`. Единственный
  судья — `grim`.

Урок для приёмки: **раздел «блокер» в отчёте проверяется первым и
своими руками, а не заносится в долги на слово.** Здесь цена ошибки
была бы фальшивый тикет на canvas-обёртку бара плюс подозрение на
форк GPUI — то есть день работы против несуществующего бага. Признак,
на который надо реагировать сразу: исполнитель утверждает, что
регрессия воспроизводится «и на чистом HEAD», но при этом ни одного
кадра `grim`, где видно, что инструмент вообще попал по цели.

### T345/T346: with_animation залипает, mv теряет deletion, бинарь без mtime-сверки — три урока приёмки (added 2026-08-21, T345/T346)

- **`with_animation` (frame-callback чейн компоситора) ненадёжен на
  пересозданных layer-shell окнах.** T346: после close→reopen левой
  панели контент навсегда оставался на `opacity(0)` — компоситор
  переставал слать `wl_callback::Done` после первого прозрачного
  кадра. Диагностический признак: сосед без анимации (рельс) остаётся
  видимым, только анимированный элемент залипает — это asymmetry, не
  «всё окно не рисуется». Канонический фикс — view-driven тикер
  (`motion::arm_enter_progress` + `motion::panel_enter_delta`), см.
  `.chronos-ops/checkpoint/ARCHITECTURE.md` §18. Правая панель несёт
  тот же паттерн по коду (T347, не воспроизведена живьём) — при
  подозрении на этот класс бага сначала грепать все места
  `with_animation` в `crates/app`, не чинить только там, где пожаловались.
- **Бинарь исполнителя без mtime-сверки — не доказательство.** На T346
  release-бинарь исполнителя был датирован РАНЬШЕ последнего изменения
  собственного `chat.rs` на 158 секунд: кадры к отчёту могли быть
  сняты со старой версии. Перед тем как доверять чужим скриншотам —
  `stat -c "%Y %n" target/release/<bin> crates/.../*.rs | sort -n`,
  бинарь обязан быть последним в списке. Не совпало — пересобрать
  самому, не переиспользовать чужой артефакт.
- **`mv` без `git mv` + поштучный `git add` теряет deletion со старого
  пути.** При переносе T346 из `active/front/` в `done/front/`
  командой `mv` (не `git mv`) с последующим `git add <новый путь>`
  старый путь остался unstaged-deletion — тикет провисел один коммит
  одновременно в `active/` и `done/`. Поймано `git status --short`
  перед следующим коммитом (T347), докоммичено. Правило: либо
  `git mv`, либо `git add -A <role>/` на весь подкаталог, не список
  путей поштучно.
