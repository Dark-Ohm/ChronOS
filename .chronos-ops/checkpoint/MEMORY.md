...
- Before any non-trivial implementation, verify against `.chronos-ops/checkpoint/ARCHITECTURE.md` / `.chronos-ops/checkpoint/REJECTED.md` (canonical docs) rather than relying on recalled prior-chat context.
...
- **Dependency policy: bleeding edge (CachyOS/Arch philosophy).** User rule 2026-07-16: держать зависимости на новейших версиях (zbus, wgpu, taffy, etc.), не пиновать старьё «для стабильности». При вырезе чужого кода (gpui-shell на zbus 5.5 и т.п.) — адаптировать под актуальные API, не тащить их версии. Исключение — только когда новая версия фактически ломает сборку (фиксировать в .chronos-ops/checkpoint/REJECTED.md с причиной и планом апгрейда).
- **SESSION_REPORT.md format is strict.** Every agent (subagent or workflow) that completes a task block MUST produce a SESSION_REPORT.md with this exact structure. Deviations are bugs:
  ...
  ## Статус .chronos-ops/checkpoint/ARCHITECTURE.md / .chronos-ops/checkpoint/REJECTED.md
...
- **Tick timer: GPUI executor, not tokio.** Plan originally said `tokio::spawn` timer, but .chronos-ops/checkpoint/REJECTED.md 2026-07-08 explicitly rejects tokio driving UI. Correct pattern: `cx.spawn()` + `cx.background_executor().timer(Duration::from_secs(1))` in a loop. `PluginManager::start_tick_loop(&self, cx: &mut App)` added to plan.
...
- **2026-08-15 / IA-волна (чекпоинт #14, только спеки):** бар-попапы разъезжаются во вкладки. Яркость+waytrogen → **Display** (сначала ошибочно налево T290; канон — справа, T296). Power profile + **Perf Gaming** → правый System (T291). **Shell Gamer** (`WorkspaceMode`) → кнопка на правой рельсе, не вкладка (T292). Notifications (T293) и Updates (T294) — правые вкладки; бар-иконки остаются входом. Updates apply **только pacman**; AUR в списке, hover = «yay в терминале». Часы → kit `Calendar` (T295), фича `gpui-component/time`. Правый dock (T289) не автооткрывает и не запирает вкладку (T221 same-tab no-op снимаем). ACP cwd = active project (T288), не cwd процесса (`packaging/`).
- **2026-08-15 / component-first:** новые контролы шелла — `gpui-component` из нашего форка `../Source/gpui-component/` (git rev `57f582f` + `[patch]` path). Ворктри `Source-wt-component` и ветка `component/feature-gates` не существуют. Окно с виджетом = `gpui_component::Root` + `OnDemand`. Input в лаунчере: `Input`/`InputState` (не `TextInput`); single-line не ест up/down/tab; `enter`/`escape` propagate. PopupMenu на Overlay: якорь в screen-space (`window.bounds().origin + event.position`), иначе click-catcher закрывает меню до клика (T275 `180fe88`).
- **2026-08-15 / Frame:** T268 = только Hide (4px bottom strip, `d572657`). Wrap — тема Appearance Hide|Wrap (T284), не пресет бара. Один мат + 3 exclusive dummy; Hyprland-конфиг не трогать. `Window::set_margin` в форке нет — recreate close+open. style парсить как строка + sanitize, не serde-enum (junction trap). wrap_inset = высота стрипа; рельса `height -= inset`.
- **2026-08-15 / Chat restore:** `restore_project_thread` грузит SQLite-ленту; ACP-клиент в HashMap зовёт `load_session` только если клиент уже есть. `ChatTab::new` всегда `create_session` — живой Hermes стартует новую сессию поверх старой ленты. Это T285, не «гейт 8 закрыт».
- **2026-08-15 / Chat chrome:** внутренний Sessions rail + Zed-ряд `✦＋☰👁⋯` + close X — срезать (T287-C). Follow (T195 `AgentFollowState` → Preview) оставить, перенести в composer-pickers-row, иконка `crates/app/assets/icons/rail-preview.svg`. Композер — самописный `text_input.rs` (`shape_line`, высота = line_count × line_height), текста нет wrap → T286 = kit Input, не хак.
- **T280 / 2026-08-14:** `ThreadStore` schema v2, one transaction, real rusqlite v1 fixture (not `ThreadStore::open` on a fresh DB). `SessionsTab` without active project is empty — never `list(None, …)`. `clear_for_project` must wipe `threads`, not only highlight. Bar `layout_config` tests live in the **binary** (`cargo test -p chronos --bins`), `--lib` finds 0.
- **T279 / 2026-08-14:** left coordinator reducers (`select_session`, `create_thread`, `switch_project`, `remove_project_scope`) are free functions on `&mut App`. They reach the live `ChatTab` through `SidePanelLeftState_.chat: WeakEntity<ChatTab>`, **not** `content_view` — a second lease of `WorkspaceView` while `on_*_event` runs is `double_lease_panic`. `ChatTab::new` still cannot be constructed in `TestAppContext` (async ACP-connect); tests call the reducer by name and assert SoT. Chat-path is `None` in unit tests and must no-op, not panic. Dead pure helpers that production never calls are reject-level (T278 lesson; `chat_layout_for_visible_width` removed in T279 r2). Committed `bd999a5`.
...
- **Luau plugin layer: `crates/luau` as standalone crate with gpui dependency.** The Element DSL lives in `crates/luau` and does Element → AnyElement conversion directly (gpui added as workspace dependency). This keeps the plan self-contained — avoids touching `crates/app` in every task. `BarWidgetAdapter` in `crates/app/src/plugin_bridge.rs` wraps a Lua VM + render callback and implements `BarWidget`. `mlua::Lua` is Arc-backed; cloning is cheap. `register_chronos_api` for MVP takes `(lua, manifest)` — event callback threading handled in manager layer. Spec: `.chronos-ops/superpowers/specs/2026-07-09-luau-plugin-layer-design.md`. Plan: `.chronos-ops/superpowers/plans/2026-07-09-luau-plugin-layer.md` (9 tasks). User-confirmed 2026-07-09.
- **BarWidgetRegistry is Vec-based with named-widget ops** (`widget.rs:13-55`). `replace_by_name(name, widget)` + `unregister_by_name(name)` added 2026-07-09 for LuaU hot-reload support. Vec chosen for order-preservation; HashMap rejected because iteration order would scramble widget layout. .chronos-ops/checkpoint/REJECTED.md 2026-07-09.
- **Tick timer must use GPUI executor, not tokio.** .chronos-ops/checkpoint/REJECTED.md 2026-07-08 "Runtime split" explicitly rejects tokio driving UI code. `bar::init()` (mod.rs:120) demonstrates the correct pattern: `cx.spawn()` + `cx.background_executor().timer()`. The luau tick dispatcher must follow this, not `tokio::spawn`. Design spec §7's "tokio::spawn timer" contradicts the accepted architecture.
- **Dev hot-reload workflow exists now (2026-07-24): `hot-lib-reloader` + `crates/hotview`.** `cargo watch --delay 0 -w crates/hotview` + `hot_lib_reloader::hot_module!` under Cargo feature `hot-reload`, ~2s save→see. Dylib name must use underscore not hyphen (`chronos_hotview`, not `chronos-hotview`), `lib_dir` must point to workspace `target/debug` (default assumes non-workspace layout). Full pitfall list: `skills/hot-lib-reloader/`. `subsecond` (Dioxus function-patching) rejected — its patch-apply API is `pub unsafe fn`, leaks `unsafe` into caller code, incompatible with this workspace's `unsafe_code = "deny"`.
- **gpui-ce's `usvg`-based SVG rasterizer does not honor `mix-blend-mode` (e.g. `destination-out`).** Cutout/hole effects in hand-written SVG icons render as solid shapes instead — confirmed via `usvg::parser::svgtree` warnings at runtime, not a build error. Design placeholder icons that rely on this CSS feature need plain solid shapes for a first pass (`crates/app/assets/icons/rail-*.svg`, T112).
- **Live UI verification IS possible in this dev environment** — a real Wayland/Hyprland session with `grim`/`hyprctl` is reachable from the assistant's shell, contradicting earlier minion reports of "no Wayland session available." Panels are layer-shell surfaces, not `hyprctl clients` toplevels — use `hyprctl layers` to find their geometry, not `hyprctl clients`. Both `side_panel_right`/`side_panel_left` have a `CHRONOS_SMOKE_SIDE_PANEL[_LEFT]=1` env-var to force pin-open without needing hover/click simulation (see `mod.rs::init` in each panel module). Clicking specific in-panel elements via `ydotool` on layer-shell surfaces is unreliable (coordinates land but the click doesn't always register on the intended widget) — don't rely on it for interactive multi-step smoke tests; static open+screenshot verification is reliable, click-driven interaction is not yet.
- **gpui-ce local patch: commit 6a7b386** on top of baseline 352c9f2. Fixes quit()-before-window-hang (calloop reset-on-entry). .chronos-ops/checkpoint/ARCHITECTURE.md §2 pinned revisions table does NOT mention this patch — documentation gap.
...
- **2026-07-22 — потеря данных: `Write`-перезапись untracked-файла без полного чтения.** При переводе `docs/orchestration/` на per-task T-ID схему (переворот оркестрации, см. .chronos-ops/checkpoint/HANDOFF.md/ARCHITECT.md) исполняющий форк сокращал `docs/orchestration/agents/bench/MIMO.md` (1462 строки), `bench/OMP.md`, `fired/AUTOHAND.md` до тонких указателей — прочитал только ~30 строк каждого и переписал файл целиком через `Write`, не проверив git-статус. Все три файла НИКОГДА не были под git (`docs/orchestration/` целиком в `.gitignore` с 2026-07-19) и, в отличие от CLINE/HERMES/GROK/ZED, не имели `archive/`-копии. Итог: история MIMO/OMP/Autohand потеряна безвозвратно — не в git log, не в trash, snapper есть только для `root`-конфига (`/home` вне снапшотов). **Правило на будущее (CLAUDE.md §Git):** перед `Write`-перезаписью любого файла целиком сначала проверять, отслежен ли он git; если нет — читать файл ПОЛНОСТЬЮ до перезаписи или не перезаписывать целиком.
...
- **Сиблинг-проект: `/home/neo/projects/chronos-ecosystem/Chronos-FM`** — отдельный GPUI-репозиторий пользователя (Launcher × Explorer, свой канон .chronos-ops/checkpoint/ARCHITECTURE.md/DECISIONS.log/ROADMAP.md), НЕ часть этого воркспейса. Пересечение с ChronOS: свой лаунчер (не решено, сливается ли с `SUPER+L` этого проекта), плагины планируются на WASM Component Model (P4, ещё не реализовано — рассматривается замена на Luau/`chronos_luau`). Подробности — в личной памяти Архитектора, не здесь (отдельный проект, не мешать каноны).
...
## Visual depth wave T128–T132 (status 2026-07-26)

User goal: **shell to daily-driver polish**, not publish-first.

| T | Layer | Status |
|---|---|---|
| **T128** | Elevated + blur tokens | **ACCEPTED WITH CAVEATS** (code in tree) |
| **T129** | Panel/popup enter-exit | **PARKED** — panels slide via `with_animation`; popups enter not trustworthy; user stop. Code: `motion.rs` |
| **T130** | Toast motion | **blocked** until T129 reopened |
| **T131** | Fork 3D primitive | **queued** after polish stable |
| **T132** | One 3D demo surface | **queued** after T131 |

**Rejected for now:** gpui-d3rs as 3D path. Further T129 thrash without new brief.

## Theme + surfaces (2026-07-26)

- Left/right panels on `Theme` tokens; right Light C roles in
  `side_panel_right/surfaces.rs` (chrome/card/well — not 1:1 mocha map).
- Toggle: Super+Shift+T, `~/.config/chronos/theme.toml`, IPC `toggle-theme`.
- Critical user-accepted dark+light grim (both panels). Polish → `.chronos-ops/checkpoint/TBD.md`.
...
- Exclusive dock on/off confirmed 2026-07-26.

## Wallpaper / waytrogen (2026-07-25…26)

- **Engine in shell:** `services/wallpaper` (awww), `wallpaper_ctl`, IPC
  `wallpaper-next` / `wallpaper-set:`.
- **T133 ACCEPTED WITH CAVEATS** — waytrogen is first-class **gallery GUI**
  (not rewrite); companion CTA when missing; resync after gallery.
- Live smoke remnants → `.chronos-ops/checkpoint/TBD.md`.
...
- **`gpui-component` принят как инфраструктура IDE-панели** (реверс
  июльского варианта C, `.chronos-ops/checkpoint/REJECTED.md` 29.07). `Input` с гейтами T156 =
