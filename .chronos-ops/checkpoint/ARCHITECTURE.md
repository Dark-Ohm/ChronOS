...
the original spec: `.chronos-ops/superpowers/specs/2026-07-08-chronos-architecture-design.md`
(brainstorming output, kept as historical record — do not edit it after the fact,
amend here instead). Rejected alternatives and why are tracked in `.chronos-ops/checkpoint/REJECTED.md`,
not duplicated here.
...
See `.chronos-ops/checkpoint/REJECTED.md` for why zed/main and crates.io were rejected.
...
`crates/app` call sites, violates the workspace deny — `.chronos-ops/checkpoint/REJECTED.md`
...
`.chronos-ops/checkpoint/REJECTED.md` 2026-07-19) converged on two standing rules for every
...
1. **Sizing: hard `.max_h(px(N)).overflow_hidden()` clip on variable-length
   ...
   `.id()`; see .chronos-ops/checkpoint/REJECTED.md]** was believed not to resolve in this fork — don't reach for
   it.
...
.chronos-ops/checkpoint/HANDOFF.md (`close_this` pattern — never call `handle.update()` for
...
- Rust side holds the widget collection as a **runtime registry**. The bar
  ...
  `.chronos-ops/checkpoint/REJECTED.md` (2026-07-09 — Bar registry: Vec, not HashMap).
...
Task 6)** — decided (.chronos-ops/checkpoint/REJECTED.md 2026-07-21: stay on `wpctl`+`pw-dump`,
...
All wave pieces landed (rationale per piece in `.chronos-ops/checkpoint/REJECTED.md`
2026-07-19/20; live status of anything newer — `.chronos-ops/checkpoint/HANDOFF.md` top block):
...
## 16. Оболочка шелла: `normal` / `wrapped` (волна 2026-08-18…20)

Принятое решение по внешней геометрии, оплаченное волной T303 → T311 →
T314 → T318 → T319 → T321 → T322 → T312. Разбор и опровергнутые гипотезы
живут в `done/design/T310-wrap-mode-invisible-and-mocha-mousse-palette.md`
(разделы A–K дополнения); сюда вынесено то, что верно в коде.

- **Два режима, `frame.toml` → `style`.** `normal` (дефолт) — оболочки
  нет: бар прижат к `y=0`, рельсы к краям экрана, инсетов нет, не
  создаётся ни одного слоя `frame_wrap_*`. `wrapped` — единая оболочка
  с апертурой: полноэкранное матте + три невидимые эксклюзивные полосы
  L/R/B. Имена в коде — `FrameStyle::Normal` / `Wrapped`.
- **`hide` / `wrap` остаются алиасами разбора навсегда.** Стиль парсится
  из строки вручную (`deserialize_style`), потому что `#[derive]` на
  неизвестном значении роняет разбор всего файла и `load()` молча
  подставляет дефолты — инцидент T268. Выпилить алиасы = схлопнуть
  пользовательские конфиги вместе со всей геометрией, без ошибки в логе.
- **Толщина — по краям, не одна на всё:** `wrap.left/right/bottom/top`.
  Мапленный рельс САМ является кромкой: при нём инсет соответствующей
  стороны = 0, конфиг игнорируется (T314). `wrap.top = 0` означает
  «следовать живой высоте бара».
- **Внутренние углы апертуры рисует кольцо с бордером** (`aperture_ring`),
  а не `rounded_*` на рельсах: скругление режет углы только наружу,
  вогнутую галтель им не построить. Форма принята владельцем после трёх
  неверных заходов — переделке не подлежит без нового решения (T318).
- **Нижней планки как отдельной сущности нет.** Низ в `wrapped` — нижний
  край матте (`wrap.bottom`); в `normal` низа нет вовсе. Секция
  `[bottom_strip]` продолжает парситься и игнорироваться (тот же T268),
  один `warn!` при не-дефолтном значении (T312).
- **Геометрия мутируется живьём, поверхности не пересоздаются.**
  `window.resize` + `set_exclusive_zone` + `cx.notify` на матте; путь
  `close + open` даёт гонку жизненного цикла в форке и протокольные
  ошибки (T314, T321, T322 — три случая одного класса).
- **Нулевой размер в `resize` убивает вейланд-соединение.** Форк клампит
  размер только на ветке `set_geometry`; в `viewport.set_destination`
  уходит сырое значение, `set_destination(0, h)` = «Size was <= 0» и
  потеря всех слоёв. При мапленном рельсе инсет штатно равен нулю —
  полосе меняется только зона, футпринт остаётся (T322).

## 17. Тема: схемы и ворота контраста

- `builtin_schemes()` — Default, Light, Solarized Dark, Mocha Mousse
  (тёмная, Pantone 17-1230). Выбор — карточками-свотчами в System
  settings; свотч рисует ЧУЖУЮ палитру, читая её из `ThemeScheme`, а не
  активную тему.
- **Любая новая схема обязана пройти WCAG-ворота автоматически:** тест
  итерирует `builtin_schemes()` и требует `text.muted` ≥ 4.5:1 на
  `bg.primary` (T317). Пастельные цвета, рассчитанные на
  противоположный фон, — повторяющиеся грабли (T239, T313).
- **Применение схемы — один путь, `theme_config::select`:** persist →
  resolve → наложение surface-настроек → `set_global` → синк
  gpui-component → `refresh_windows`. Переключение темы НЕ сбрасывает
  `surface_alpha` / `blur_enabled` (T266).

## 18. Enter-анимация окон панели: view-driven тикер, не `with_animation` (T346, 2026-08-21)

- **`with_animation` (компоситор frame-callback чейн) ненадёжен на
  layer-shell окнах Hyprland.** На пересозданном (закрыт→открыт) окне
  тик может залипнуть на нулевом кадре — элемент навсегда остаётся
  `opacity(0)`, компоситор перестаёт слать `wl_callback::Done` на
  визуально-статичную прозрачную поверхность. Диагностический признак:
  анимированный элемент пропадает после close/reopen, соседний
  неанимированный элемент (например рельс) остаётся видимым — рельс не
  использует `with_animation` вообще, отсюда асимметрия.
- **Канонический фикс — view-driven тикер** `motion::arm_enter_progress`
  (фон-экзекутор, `Duration`-таймер, пишет прогресс в поле вью +
  `cx.notify()` на каждый тик) вместо `with_animation`. Уже несёт
  `volume_popup` (переехал раньше по этой же причине), с T346 — и
  `side_panel_left` (`workspace_view.rs`, `tabs/chat.rs`).
- **Хелпер `motion::panel_enter_delta(enter_t, armed)`:** неармленная
  панель (тикер не запущен/сброшен) рендерится на `delta = 1` — полная
  непрозрачность, `opacity(0)` структурно недостижим, кроме случая
  реально отсутствующего окна. Армленная — клампит эйзд-прогресс тикера.
  Армится на переходе rail-only → visible, сбрасывается на collapse
  (реплей enter на каждом раскрытии).
- **`side_panel_right/view.rs:594` несёт идентичный `with_animation` +
  `apply_enter_from_right` паттерн** — тот же класс риска, флаг по коду,
