# HANDOFF-LOG — журнал сессий Архитектора (АРХИВ, не первое чтение)

**Что это.** Хронологический журнал: чекпоинты сессий, разборы приёмок,
разбор багов по дням. Рос до 3973 строк и стал нечитаем целиком —
2026-08-18 разрезан: **актуальное состояние переехало в
`.chronos-ops/checkpoint/HANDOFF.md`**, этот файл остался журналом.

**Как этим пользоваться.** Не читать подряд. Грепать по T-ID
(`grep -n 'T265' HANDOFF-LOG.md`), по дате (`2026-07-29`) или по симптому
(`remove_window`, `ydotool`, `blur`). Порядок внутри — обратно-хронологический
сверху и мешанина ниже; заголовки уровня `##`/`###` датированы.

**Приоритет источников.** При расхождении с `HANDOFF.md` побеждает
`HANDOFF.md`. Архитектурные решения — `ARCHITECTURE.md`, отклонённое —
`REJECTED.md`, реестр задач — `.chronos-ops/MIGRATION.md`. Этот файл ниже
их всех: он фиксирует, как было, а не как есть.

**Ничего не удалено при разрезе** — всё, что было в `HANDOFF.md` на
2026-08-18, лежит ниже дословно, включая шапку того дня.

---

# HANDOFF (содержимое на момент разреза 2026-08-18)
