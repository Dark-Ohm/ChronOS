...
- Before any non-trivial implementation, verify against `docs/ARCHITECTURE.md` / `docs/DECISIONS.log` (canonical docs) rather than relying on recalled prior-chat context.
...
- **Dependency policy: bleeding edge (CachyOS/Arch philosophy).** User rule 2026-07-16: держать зависимости на новейших версиях (zbus, wgpu, taffy, etc.), не пиновать старьё «для стабильности». При вырезе чужого кода (gpui-shell на zbus 5.5 и т.п.) — адаптировать под актуальные API, не тащить их версии. Исключение — только когда новая версия фактически ломает сборку (фиксировать в docs/DECISIONS.log с причиной и планом апгрейда).
...
- **Мульти-монитор: пультовый монитор + холст.** Один монитор — «пультовый» (top bar, side panels, попапы, лаунчер — вся управляющая поверхность). Второй — «рабочий холст» под окна (Hyprland рулит) + desktop-виджеты (абсолютное позиционирование, `Layer::Background`). Назначение пультового по UUID в конфиге (`~/.config/chronos/`), т.к. `cx.primary_display()` на Wayland/Hyprland возвращает `None` (нет канонического primary). Побочный выигрыш: consolidation убирает долг по 8 попапам (Zed №3 баг). docs/DECISIONS.log 2026-07-19.
- **Светлая тема: собственная айдентика.** Не Latte-инверсия. Направление: атом · киберпанк · микрочип-трассы · сигилы/сакральная геометрия · Хронос. Неон живёт в линиях/деталях (бордеры, glow-рёбра, акцентные штрихи), НЕ в заливке фона. Поверхность — спокойная holodная сине-лавандовая база (`#dde0f2`/`#e6e9fa`, текст `#2c2e4a` индиго). Эталон: `docs/design/Project Switcher.dc.html` вариант "Light C". docs/DECISIONS.log 2026-07-19.
...
- **Tick timer: GPUI executor, not tokio.** Plan originally said `tokio::spawn` timer, but docs/DECISIONS.log 2026-07-08 explicitly rejects tokio driving UI. Correct pattern: `cx.spawn()` + `cx.background_executor().timer(Duration::from_secs(1))` in a loop. `PluginManager::start_tick_loop(&self, cx: &mut App)` added to plan.
...
- **BarWidgetRegistry is Vec-based with named-widget ops** (`widget.rs:13-55`). `replace_by_name(name, widget)` + `unregister_by_name(name)` added 2026-07-09 for LuaU hot-reload support. Vec chosen for order-preservation; HashMap rejected because iteration order would scramble widget layout. docs/DECISIONS.log 2026-07-09.
- **Tick timer must use GPUI executor, not tokio.** docs/DECISIONS.log 2026-07-08 "Runtime split" explicitly rejects tokio driving UI code. `bar::init()` (mod.rs:120) demonstrates the correct pattern: `cx.spawn()` + `cx.background_executor().timer()`. The luau tick dispatcher must follow this, not `tokio::spawn`. Design spec §7's "tokio::spawn timer" contradicts the accepted architecture.
...
- **gpui-ce local patch: commit 6a7b386** on top of baseline 352c9f2. Fixes quit()-before-window-hang (calloop reset-on-entry). docs/ARCHITECTURE.md §2 pinned revisions table does NOT mention this patch — documentation gap.
...
- **2026-07-22 — потеря данных: `Write`-перезапись untracked-файла без полного чтения.** При переводе `docs/orchestration/` на per-task T-ID схему (переворот оркестрации, см. docs/HANDOFF.md/ARCHITECT.md) исполняющий форк сокращал `docs/orchestration/agents/bench/MIMO.md` (1462 строки), `bench/OMP.md`, `fired/AUTOHAND.md` до тонких указателей — прочитал только ~30 строк каждого и переписал файл целиком через `Write`, не проверив git-статус. Все три файла НИКОГДА не были под git (`docs/orchestration/` целиком в `.gitignore` с 2026-07-19) и, в отличие от CLINE/HERMES/GROK/ZED, не имели `archive/`-копии. Итог: история MIMO/OMP/Autohand потеряна безвозвратно — не в git log, не в trash, snapper есть только для `root`-конфига (`/home` вне снапшотов). **Правило на будущее (CLAUDE.md §Git):** перед `Write`-перезаписью любого файла целиком сначала проверять, отслежен ли он git; если нет — читать файл ПОЛНОСТЬЮ до перезаписи или не перезаписывать целиком.
...
- Critical user-accepted dark+light grim (both panels). Polish → `docs/TBD.md`.
...
- Live smoke remnants → `docs/TBD.md`.
...
- **`gpui-component` принят как инфраструктура IDE-панели** (реверс
  июльского варианта C, `docs/DECISIONS.log` 29.07). `Input` с гейтами T156 =
