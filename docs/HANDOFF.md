...
**Обновлено: 2026-07-31 (вечер).** За день закрыты **два слайса Shell-IDE
целиком** и открыт третий:
...
- **слайс 4** (рабочий стол разработчика): план написан, T175 (разведка),
  T176 (вкладка Files) и T177 (вкладка Terminal) приняты — **волна 1
  закрыта**, поле пустое.
