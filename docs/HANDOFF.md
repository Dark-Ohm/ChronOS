...
**T166 (единственный резолвер вывода + hotplug) — в поле.** Четыре прямых
`cx.primary_display()` (`bar/mod.rs:223`, `side_panel_left/mod.rs:55`,
`hover_strip.rs:42`, `dock/context_menu.rs:129`) переезжают внутрь
`monitor.rs`. Идёт параллельно, зона не пересекается.
