...
**Обновлено: 2026-07-31 (день) — слайс 2 Shell-IDE закрыт в коде: T164, T165
и T166 приняты, в поле QA-смок T167. Ниже свежий блок; всё после него —
история.**
...
**T167 (QA-смок слайса 2) — ПЕРВЫЙ ЗАХОД НЕ ПРИНЯТ, эррата `621103b`.**
Три слота из девяти закрыты. Разбор:

- **P6 PASS** — снёс `monitor.toml`, шелл поднялся, `bar::init` авто-назначил
  DP-1 и записал uuid, вотчер подхватил. Второй дефект T166 закрыт живьём.
- **P7 cold-boot PASS** — ложного «reconnected» на старте нет (грепом по 51
  строке `session-2.log`).
- **P7 fake-loss — на самом деле PASS, а не «частично».** Исполнитель написал
  «не могу сказать, что в кадрах видна toast — vision 12px врёт». Я открыл
  `/tmp/chronos-t167-evidence/p7-{disconnect,reconnect}-toast.png` глазами:
  оба тоста в кадре, крупным текстом, «Display deadbeef… disconnected. Shell
  on fallback.» и «Display 09e7b298… is back.» **Он занизил собственный
  результат, потому что не открыл свой же файл.** Правило T162 звучит «не
  заводи дефект по тексту 12 px — **открой кадр и посмотри**»: это требование
  посмотреть, а не разрешение не смотреть.
- **Блокер предпосылки:** «единственный путь переключить режим — править
  `workspace.toml` + kill+restart». Ложь. Живой IPC
  `set-workspace-mode:<mode>` в `$XDG_RUNTIME_DIR/chronos.sock`, диспетч
  `ipc/mod.rs:143-150`, дебаунс 200 мс — им же закрывался слайс 1 в T162.
  **Один запуск шелла закрывает P1–P4.** Из-за ложной посылки пять пунктов
  не сделаны, а в отчёте пять раз «за архитектором» — сбор улик это и есть
  работа QA, вердикт мой, факты его.
- Фактические ошибки отчёта: «`resolve_pinned` в Gamer merg-ит user pins» —
  merge нет, `dock/config.rs:86` это `Gamer => default_pinned_for_mode(mode)`;
  рецепт `scenes.toml` в отчёте невалиден как TOML (`[last.gamer] mode = "..."`
  в одну строку) — выполни он его, записал бы дефект в T164 на пустом месте;
  `DuckMenuState`; выдуманный «pult uuid = HNAW700095».
...
**Дальше:** слайс 2 в коде закрыт целиком (T164 + T165 + T166). В поле —
**T167 на эррате** (второй заход): восемь пунктов, из них P4 (`scenes.toml`
переживает переключения побайтово) и P6 (вотчер поднимается без
`monitor.toml`) закрывают ровно те дефекты, за которые T164 и T166 не приняли
с первого раза; P7 проверяет пропажу вывода через подмену uuid, а не через
компоновщик; P8 воспроизводит панику `DockMenuState`. После смока — выбор
слайса 3 из восьми (§14 спеки, каждый требует своего плана).
