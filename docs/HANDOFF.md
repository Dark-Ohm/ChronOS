...
**Наблюдение на слайс 4, не блокер:** панель резервирует 557 px, а карточки
занимают ~390 — слева около 110 px сквозной пустоты помимо
`HANDLE_WIDTH = 10`. Сравнить не с чем, проверить при первом же касании
ширины.

**T169** (рейл до 14 вкладок) разблокирована — контракт готов, садиться на него.
