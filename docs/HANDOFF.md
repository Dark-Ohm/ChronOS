...
Инвариант слоя: «Пуск» не кроют ни клиенты, ни наши Overlay-панели —
меню само Overlay, при open закрыть левую панель (правую — если
пересекается). Голый AnchoredPopup от бара (Top) панелями накроется.
