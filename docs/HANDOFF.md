...
**Долг:** кадр рейла (Developer 14 / Gamer 7) не снят — пультовый вывод под
фуллскрин-игрой, исполнитель правильно остановился. Уходит в QA-смок
слайса 3; там же проверить, что `fill-opacity` рендерится, в отличие от
`mix-blend-mode`.
...
**В поле:** T171 (ширина панели как свойство вкладки) и T172 (иконки без
`mix-blend-mode`) — зоны не пересекаются, можно параллельно. T172 целиком
проверяется отрисовкой SVG, живой шелл не нужен.
