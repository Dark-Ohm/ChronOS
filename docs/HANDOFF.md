...
**Обновлено: 2026-07-31 (ночь).** За день закрыты **три слайса Shell-IDE**:

- **слайс 2** (сцена и композиция): T164, T165, T166, T167 — закрыт смоком;
- **слайс 3** (модуляризация правой панели): T168–T173 плюс T174 —
  контракт вкладки, рейл до четырнадцати, ширина по вкладке, глобалы дока,
  иконки без `mix-blend-mode`;
- **слайс 4** (рабочий стол разработчика): **закрыт кодом целиком** —
  T175 (разведка), T176 (Files), T177 (Terminal), T178 (Build/Logs),
  T179 (Preview), T180 (markdown без сети). В поле только T181 — QA-смок,
  закрывающий слайс.
...
**Механика, на которой потерялись двое, включая меня.** Панель
раскрывается **в два приёма**: клик по иконке рейла только *выбирает*
вкладку, ширина остаётся 54 px; контент раскрывает кнопка `⊞`/`⊟` в самом
низу рейла (`x ≈ 2539, y ≈ 1414`; `ydotool` — половинные `1269 707`).
Признак в логе — `apply per-tab width … content_open=false`. Без этого
любые кадры «вкладки» выходят пустыми, и живой прогон превращается в
фикцию.
