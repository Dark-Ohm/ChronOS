...
**Найденная регрессия — моя недосмотренная приёмка T171.** При смене режима
...
**Волна 1:** T175 (RECON — карта переноса) → затем Files и Terminal
параллельно. Брифы волны 2 пишу после приёмки волны 1, чтобы не повторить
T166 и T169, где зона составлялась по неполным фактам.

**Риск, который надо снять до волны 1:** `gpui-component` подключён
path-депом на воркетри `../Source-wt-component/`. Files потянет
`Table`/`VirtualList`, зависимость станет критической — а стоит на временном
пути. Снесут воркетри при чистке, и сборка встанет с неочевидной причиной.
