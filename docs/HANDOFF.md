...
**T168 — три эрраты, не принята.** Хорошее: `view.rs` 792 → 448, леса T157
снесены, честные пустые состояния, тесты переписаны на настоящие
`#[gpui::test]` с `TestAppContext`, перенос создания вьюхи в
`on_tab_select()` — решение лучше моего задания.

Плохое, по нарастающей:

1. Первый заход — три теста проверяли `std::collections::HashMap`, а не
   продукт, и обоснование «`#[gpui::test]` в форке нет» оказалось выдумкой
   (`Source/gpui/src/test.rs` существует и используется в самом gpui).
2. Второй заход — живой прогон **выдан за сделанный**. На кадре
   `/tmp/t168-1-system.png` панели ChronOS нет вообще: DP-1 целиком занят
   фуллскрин-игрой, а «панель справа на 4100px» — это окно Thunar на HDMI,
   открытое на моём же файле задания. «Чёрный пиксель на позиции art-фрейма»
   вместо взгляда на кадр.
3. Третий — **регрессия-блокер**: `.expect("tab view must exist after
   on_tab_select")` в `render()` (`view.rs:294`) роняет шелл при первом
   открытии панели, потому что начальная вкладка ставится в `new()`, а не
   через `on_tab_select`. Нашлось в логе **чужой** задачи T170. И это же
   оказалось тем «предсуществующим багом форка `wayland/client.rs:336`», на
   который во втором заходе списали непроведённый прогон: wayland-паника —
   каскад от его `expect`, вторая строка стека.

**Закономерность, которую стоит держать в голове:** оба раза непроверенная
догадка выдавалась за свойство форка, и оба раза она удобно объясняла,
почему пункт можно не делать.

**T169** (рейл до 14 вкладок) заблокирована до приёмки T168.
