...
**Обновлено: 2026-07-31 (день) — слайс 2 Shell-IDE в работе: T164 и T165
приняты, T166 в поле. Ниже свежий блок; всё после него — история.**
...
**T166 (единственный резолвер вывода + hotplug) — ПЕРВЫЙ ЗАХОД НЕ ПРИНЯТ,
эррата выдана (`2106862`), код в `a238ada`.** Три пункта:
- **Блокер:** `pult_display_info` объявлена, задокументирована («use this
  instead of manual `find_display().or_else(primary_display)`») и **не
  вызывается ни разу**. В четырёх поверхностях `.or_else(|| cx.primary_display())`
  просто удалён — они делают `pult_display(cx).and_then(find_display)` без
  финального фолбэка. «Фолбэк в одном месте» существует только на бумаге.
  Компилятор промолчал: `pub fn` в модуле, экспортированном из `lib.rs`, под
  dead-code не попадает — **это ловушка, стоит помнить**.
- Hotplug-вотчер (`start_hotplug_watcher`, опрос раз в 3 с) делает ранний
  выход при пустом `chrome_monitor`, а `monitor::init` (`main.rs:82`) идёт
  ДО `bar::init`, который и делает авто-назначение. На чистой машине
  наблюдение не запустится всю сессию.
- Живой прогон не сделан вообще.

**Мой недосмотр в этой задаче:** в бриф я выписал четыре места с
`cx.primary_display()`, а их **семь** — снял греп с `| head -20` и обрезал
вывод. Остальные: `side_panel_right/{view.rs:346, mod.rs:135,
hover_strip.rs:44}` и `desktop_terminal/mod.rs:25` (последний зовёт
`cx.primary_display()` напрямую, без `pult_display` вообще). Исполнитель их
нашёл и честно вынес в «вне моей зоны». Зона T165 закрыта, поэтому эррата
отдаёт ему все четыре. **Правило: греп для зоны — без `head`, с `| wc -l` и
сверкой числа строк.**

Засчитано сразу и трогать не надо: `largest_display_index` вынесена в чистую
функцию с тестом на равные площади, `push_internal` для внутренних
уведомлений с честно описанным ограничением (инжектированное уведомление
сотрётся следующим апдейтом от fdo-демона, `state::watch` заменяет `current`
целиком). Ложное обвинение снял сам: `let _ = cx.update(...)` в вотчере —
не проглоченная ошибка, в форке `AsyncApp::update` возвращает `R`, не
`Result` (`async_context.rs:163`, находка T160).
...
**Дальше:** приёмка T166 → QA-смок слайса 2 (T167, завести при приёмке) →
выбор слайса 3 из восьми (§14 спеки, каждый требует своего плана).
