...
по-прежнему падает в `create_session` fallback с новым id. **T286/T287
остаются заблокированы** — нет обходного публичного пути без форка SDK
(явно запрещено брифом) или архитектурного решения не использовать
`ActiveSession` вовсе (прямой JSON-RPC поверх `add_dynamic_handler` —
не оценивал, отдельное решение). Рекомендация — upstream issue в
`zed-industries/agent-client-protocol` за возврат публичного
`attach_session`/load-билдера. Не блокер сессии, справочный пункт.
