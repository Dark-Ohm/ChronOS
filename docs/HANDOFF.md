...
T265-H код-зависимости закрыты, **выдача BLOCKED на мокап владельца**
(рисует сейчас). Не клонировать OSD. Не параллелить с T297 (`app_menu.rs`).
Слой: меню Overlay, при open закрыть левую панель. Голый AnchoredPopup
от бара (Top) панелями накрывается.
