...
> **`.chronos-ops/design/` — осторожно.** Мокапы компонентов (`Top Bar.dc.html`, попапы)
...
## Контент ПОВЕРХ заливки — только через `on_fill()`

`chronos_ui::on_fill(fill)` (`theme/mod.rs`) — контрастный передний план
для того, что лежит НА насыщенной заливке (`accent.*`, `status.*`):
цифра в бейдже, кружок тумблера. Выбор идёт по светлоте САМОЙ ЗАЛИВКИ, а
не по активной схеме, и возвращает полюса палитры (`#11111b` / `#cdd6f4`),
а не чистые `#000`/`#fff`.

Почему нельзя брать `theme.text.primary`: он переворачивается вместе со
схемой, а заливки — нет (акцент `#007acc` одинаков в обеих). Живые
случаи 2026-07-20: цифра бейджа уведомлений почти-белым на светло-розовом
`status.error` (плыла уже в тёмной) и кружок тумблера gaming-mode
светло-серым на светлом треке (пропадал в Light C).

## Статусы: тёмная — Mocha, светлая — Latte

`status.*` НЕ общие для схем. Пастельные Catppuccin **Mocha**
(`#f9e2af` и родня) рассчитаны на тёмный фон: живой смок 2026-07-20
показал, что `↑ 19` в виджете обновлений на светлом баре практически
невидим. Светлая схема несёт **Latte**-статусы (`d20f39` / `df8e1d` /
`40a02b` / `1e66f5`) — те же смыслы, контраст под светлую поверхность.
Тест `light_scheme_status_is_latte_not_mocha` это стережёт.
...
- **Акцент живёт в линиях/glow/марках, НЕ в заливке.** `#007acc` — сигилы,
  ...
  правило в светлой теме — см. `.chronos-ops/checkpoint/REJECTED.md` 2026-07-19.)
