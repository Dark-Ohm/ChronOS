...
- **Cross-thread wake (REQUIRED, live smoke-test — not a unit test):** `futures_signals::Mutable::set()` is thread-safe by construction — `set()` → `ChangedWaker::wake()` stores the waker in a plain `std::sync::Mutex<Option<Waker>>` and calls `Waker::wake()` (verified in `futures-signals` 0.3.34 `src/signal/mutable.rs`); it holds no runtime handle, so a `set()` from a foreign `std::thread` (the Compositor listener) correctly wakes the GPUI-side `Signal` consumer. **However**, this path is only exercised when the producer and consumer run on *different* executors (Compositor `std::thread` → GPUI `cx.spawn`). An isolated single-runtime unit test does NOT cover it. Therefore the Compositor implementation task MUST include a **live smoke-test**: under a real Hyprland session, `hyprctl dispatch workspace 2` (or equivalent) → assert the bar/widget `CompositorState` updates via `watch()`. This is the guard against the "works by design, fails on first live run" class of bug. Mark it `#[ignore]` (needs a session) but it is a required acceptance gate, not optional.
