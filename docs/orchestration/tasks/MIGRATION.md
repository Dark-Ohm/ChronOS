...
| T163 | **FRONTEND** | `bar.toml`: новый builtin-виджет не доезжает до существующего пользователя | **OPEN** — вскрыто приёмкой T161. Нужна одноразовая миграция при загрузке, но так, чтобы **сознательно удалённый виджет не воскресал** — это главный тест. Три варианта в брифе (версия схемы / список известных имён / только уведомление), рекомендация — список известных | `active/T163-bar-layout-new-builtin-migration.md` |
