...
| T164 | **BACKEND** | Scene-модель: состояние вне панелей, персист `~/.config/chronos/scenes.toml`, восстановление последней сцены режима | **OPEN** — открывает слайс 2 (§14 п.2 спеки). Вариант **B** утверждён пользователем 2026-07-31: сцена помнит только состояние шелла (режим, вывод по UUID, набор/активная вкладка рейла, док); внешние окна — вариант C, следующая итерация. Отсюда требование: `version = 1` и зарезервированная секция `[scene.windows]` с первого коммита, парсер обязан её переживать — второй T163 с миграцией живого конфига мы не устраиваем. Образец — `workspace_mode.rs` целиком. Красная линия: ни одного нового вызова `workspace_mode::set/toggle` | `active/T164-scene-state-model.md` |
