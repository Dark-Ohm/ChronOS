...
| T165 | **FRONTEND** | Композиция по режиму: набор вкладок правого рейла и состав дока следуют Developer/Gamer | **OPEN, после приёмки T164** — садится на его API сцены. Сегодня режим не меняет ничего, кроме подписи виджета. Порядок разрешения: оверрайд сцены → дефолт режима → текущий дефолт. `PanelTab::ALL` (жёсткий массив на 10, `tabs.rs:56`) остаётся как «все вкладки» для тестов покрытия, набор режима — второе понятие; тест `assert_eq!(ALL.len(), 10)` придётся переписать. Позиции общих инструментов между режимами не меняются (§5) — это пункт приёмки, а не пожелание. Панель при смене режима не закрывается | `active/T165-mode-driven-composition.md` |
