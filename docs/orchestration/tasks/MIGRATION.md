...
| T166 | **BACKEND** | Единственный резолвер пультового вывода + пере-резолв при hotplug | **OPEN, параллельно T164/T165** — зоны не пересекаются. `pick_display()` в notifications/system_popup/volume_popup/history_popup/dock уже обёртки над `monitor::pult_display`; остались четыре прямых `cx.primary_display()` — `bar/mod.rs:223`, `side_panel_left/mod.rs:55`, `side_panel_left/hover_strip.rs:42`, `dock/context_menu.rs:129`. Фолбэк переезжает внутрь `monitor.rs` (§3.6: «Surfaces must not call `cx.primary_display()` directly»). Ловушка в брифе: `pult_display()` пишет на диск как побочный эффект (авто-назначение, `monitor.rs:97`). Плюс hotplug: пере-резолв, видимое уведомление, состояние сцены не теряется | `active/T166-pult-display-consolidation.md` |
