...
| T169 | **FRONTEND** | Рейл до четырнадцати вкладок по §4.1 | **ЭРРАТА, первый заход не принят** (коммит `d3326bc`). Каталог вырос честно: `ALL.len() == 14`, порядок обоснован (правка → результат → диагностика) со ссылкой на §12, `for_mode(Gamer)` не тронут — 7 вкладок с целой группой настроек, контракт T165 сохранён. Тесты **расширены, а не ослаблены**: `all_has_fourteen_tabs_in_fixed_order` проверяет все четырнадцать индексов вместо прежних трёх, добавлен `developer_settings_group_matches_gamer_settings_group_order` — защита §5, которой я не просил. Восемь новых тестов, 252 зелёных, прогнал сам. Зона соблюдена безупречно, **закоммитил сам — первый раз за семь задач**. **Блокер:** `rail-preview.svg` нарисована через `mix-blend-mode: destination-out`, который наш рендерер игнорирует (см. T172) — в шелле станет третьим неотличимым прямоугольником рядом с `Terminal` и `HyprlandBinds`. Приём скопирован у существующих иконок без проверки, работает ли он, при том что отчёт сам зафиксировал предупреждение `usvg` и списал его на системный шум. Живой прогон не сделан легитимно — DP-1 под фуллскрин-игрой, в игру не полез | `active/T169-rail-fourteen-tabs.md` |
