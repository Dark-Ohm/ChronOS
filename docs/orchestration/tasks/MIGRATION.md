...
| T174 | **FRONTEND** | Ширина не следует за вкладкой при автоматической смене на System | **OPEN.** Регрессия найдена QA в T173. `side_panel_right/view.rs:263-272`: когда активная вкладка уходит из набора режима, `render` переводит `active_tab` на System и зовёт `ensure_tab_view`, но **ширину не применяет** — `on_tab_select` этот путь не проходит, а именно он в T171 вызывает `ensure_content_width(active_tab_width(...))`. Живой замер: Editor в Developer даёт слой `w=560`, переключение в Gamer оставляет `w=560`, хотя активной стала System с `preferred_content_width = 400`. Панель при этом не закрывается и фокус уходит верно — ломается только ширина | `active/T174-width-follows-auto-tab-switch.md` |
...
| T176 | **FRONTEND** | Вкладка Files: порт слоя FS и своя узкая вьюха | **OPEN, параллельно T174/T177.** По карте разведки T175: переносится `list_dir_sync` + `FileEntryDto` + сортировка (~320 строк, ноль GPUI), вьюха пишется своя по образцу `SystemTab` под ширину 440. `fs/ops.rs` в задачу не входит — §4.1 держит файловые операции за отдельной capability. Кровное: `list_dir_sync` блокирующий, а в Chronos-FM его зовут прямо в обработчике — у нас обязан уходить в `cx.background_spawn` (правило Runtime split). `VirtualList` разрешено взять только по замеру: `gpui-component` висит path-депом на воркетри `../Source-wt-component/`, и лишняя привязка к временному пути не нужна | `active/T176-files-tab.md` |
