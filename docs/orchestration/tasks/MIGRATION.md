...
| T172 | **FRONTEND** | Иконки с `mix-blend-mode` рисуются сплошными пятнами | **OPEN, параллельно T169/T171.** Найдено архитектором при приёмке T169: `usvg` под GPUI **не поддерживает** `mix-blend-mode: destination-out` — помеченный элемент не вычитается из подложки, а заливается поверх неё. Доказано живым кадром `/tmp/t168-live/3-system-back.png` (зум рейла 400 %): `rail-terminal.svg` содержит рамку и `>_`, `rail-binds.svg` — пять клавиш и пробел, а на экране обе — **голые прямоугольники**. Контроль: рендер тех же файлов с механически вырезанным `mix-blend-mode` совпадает с живым кадром. Приём используется в шести иконках (`terminal`, `binds`, `editor`, `api` + новые `preview`, `build` из T169 — те чинятся в самой T169). Замена — `fill-rule="evenodd"` внутри одного `<path>` (usvg это умеет) либо непересекающиеся примитивы по образцу `rail-lsp.svg`. Дефект целиком проверяется отрисовкой SVG, живой шелл не нужен | `active/T172-icons-blend-mode-broken.md` |
