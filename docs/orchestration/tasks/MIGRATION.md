...
| T173 | **QA** | Живой смок слайса 3: контракт вкладки, четырнадцать вкладок, ширина по вкладке | **OPEN.** Закрывающая задача слайса. Восемь пунктов; часть живого прогона архитектор уже закрыл при приёмке (14/7 иконок, ширина 400/560/400, System цела, меню дока, ноль паник) — в брифе это перечислено как «не переделывать». Главный пункт — **долг ещё со слайса 2**: вкладка уходит из набора режима → фокус на System **без закрытия панели** (в T167 сценарий не разыгрывался, на обоих кадрах активной была System). Плюс: все четырнадцать пустых состояний глазами, ленивость и кэш вьюх по логу `lazy-create`, память ресайза по вкладкам, оверрайд `rail_tabs` из `scenes.toml` на новом наборе с побайтовой сохранностью файла, док после T170, и замер нагрузки при четырнадцати вкладках (§10) — `resolve_for_mode` возвращает свежий `Vec` каждый рендер, долг с T165 | `active/T173-right-panel-slice-smoke.md` |
