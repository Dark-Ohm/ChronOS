...
| T169 | **FRONTEND** | Рейл до четырнадцати вкладок по §4.1 | **ЗАБЛОКИРОВАНА до приёмки T168.** Добавляются `Preview`, `Inspector`, `Build`, `SourceControl` в группу рабочих инструментов; Developer 14, Gamer остаётся 7 с целой группой настроек (строка 149 спеки). Иконок под них в `crates/app/assets/icons/` **нет** — нужны четыре новых `rail-*.svg` в существующем языке форм (`viewBox="0 0 256 256"`, `fill="currentColor"`, 240–675 байт геометрии). Содержимого новых вкладок не делаем — это слайс 4 | `active/T169-rail-fourteen-tabs.md` |
