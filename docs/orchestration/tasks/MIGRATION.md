...
| T175 | **RECON** | Разведка: что переносится из Chronos-FM во вкладку Files | **OPEN, первая задача слайса 4.** `explorer/` в Chronos-FM — 3566 строк под отдельное приложение со страницами и роутингом; нам нужна одна вкладка шириной ~440 px по контракту `TabContent`. Нужна карта «берём / режем / переписываем» по каждому файлу с `file:line`, список отсутствующих у нас зависимостей, требуемые компоненты `gpui-component` (цена измерена в T157) и конкретные препятствия по контракту вкладки. Проверено архитектором заранее: форк один и тот же, разъезд два коммита в одну сторону; `chronos-fm-services/src/fs/` — 584 строки чистого бэкенда. Разведке разрешено заключить, что перенос дороже написания с нуля — но с цифрами | `active/T175-chronos-fm-port-recon.md` |
