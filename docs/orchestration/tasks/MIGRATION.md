...
| T171 | **FRONTEND** | Ширина правой панели как свойство вкладки | **OPEN, после T169.** Замечание пользователя при приёмке T168: 557 px слишком широко, растягивание нужно только вкладке с блокнотом. Корень найден: `DEFAULT_CONTENT_WIDTH = 560` (`mod.rs:42`) — **одна константа на все вкладки**, задранная под самую широкую, платят все остальные. Живой замер: слой `w=557`, карточки System внутри ~390, слева ~110 px сквозной пустоты сверх `HANDLE_WIDTH = 10` — пятая часть пультового вывода резервируется зря. Решение: предпочитаемая ширина уезжает в контракт вкладки (сделан в T168), `DEFAULT_CONTENT_WIDTH` остаётся фолбэком для необъявившихся. Решение архитектора по развилке: **пользовательский ресайз запоминается на вкладку**, иначе одно перетаскивание края ломает раскладку всем четырнадцати; персист на диск НЕ в этой задаче — место есть (`scenes.toml`), но писать должен `SceneManager`, которого ещё нет | `active/T171-tab-preferred-width.md` |
