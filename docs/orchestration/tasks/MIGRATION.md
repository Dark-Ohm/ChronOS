...
| T168 | **FRONTEND** | Контракт вкладки правой панели + System в свой модуль + снос лесов T157 | **OPEN, первая задача слайса 3.** `view.rs` 792 строки, девять вкладок из десяти рисуют `"{label} — coming soon"` (`view.rs:582-596`), контент есть только у System. Леса T157 сидят внутри System: `measure_input`/`measure_table`/`measure_vlist`, `DemoTableDelegate`, `DemoVirtualList` (`view.rs:22-24,82-93,447-489,686-792`) — замер (+1.96 MiB) давно принят, а демо-таблица до сих пор рисуется у пользователя. Решение плана: ленивый реестр `AnyView` на вкладку, кэш без сброса при смене режима, точка входа одна. `Root` и `gpui_component::init` не трогать (без `Root` `Input` паникует — находка T158) | `active/T168-panel-tab-contract.md` |
