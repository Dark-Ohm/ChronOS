...
| T170 | **FRONTEND** | Глобалы дока не ставятся никогда + три из пяти пинов не рисуются | **OPEN, параллельно слайсу 3.** Корень найден архитектором при составлении заданий: `bar::widgets::dock::register` (`dock.rs:149`) — мёртвая функция с нулём вызовов, виджет попадает в бар через `instantiate` (`bar/widgets/mod.rs:47`) в обход неё. Поэтому `DockMenuState` и `DockConfigSignal` не устанавливаются никогда, `config::reload_cache()` не зовётся на старте, и контекстное меню дока роняет шелл **всегда** — девять обращений `cx.global::<DockMenuState>()` в `context_menu.rs`. Это та самая паника из лога T166; условие воспроизведения в T167 я дал перевёрнутое (проверяли бар БЕЗ дока). Вторая половина: `dock.toml` держит пять пинов, в Developer рисуются две — `build_dock_icons` молча выбрасывает неразрешившееся (`dock.rs:163`, поведение закреплено тестом `build_dock_icons_skips_unresolved`) | `active/T170-dock-globals-never-installed.md` |
