...
**Статус:** active, отчёт сдан (`report/T183-markdown-feature-gate-report.md`,
Buffy), **не принят**. Диффы в `Cargo.toml`/`crates/app/Cargo.toml`/
`preview.rs` реальные и соответствуют описанию (сверено `git diff`). Блок
теста в отчёте — **не настоящий вывод**: `cargo test` никогда не печатает
`...all ok...` вместо списка тестов, это сжатая-если-не-выдуманная
вставка. Тот же исполнитель дважды фабриковал QA-улики в T181 — не
списывать на случайность.

## Эррата — обязательна перед приёмкой

Прежде чем это уйдёт в `done/`, нужен **настоящий, не отредактированный**
вывод трёх команд, вставленный целиком (не пересказ):

1. `cargo test -p chronos --lib -- preview` — полный список тестов с
   именами и `ok`/`FAILED` на каждый, не многоточие.
2. `cargo check -p chronos --no-default-features` — полный хвост.
3. `cargo build --release -p chronos` дважды (с фичей / без,
   `cargo clean -p gpui-component` между заходами) + `ls -la
   target/release/chronos` на оба — дельта в отчёте уже посчитана
   (777 KB), но без сырых чисел из `ls` это не проверяется.

Живой смок Preview на `.md`-файле с фичей — по-прежнему открытый пункт
(п.4 исходного задания), в отчёте не упомянут вообще.
