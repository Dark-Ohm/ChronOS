...
> **⚠ SUPERSEDED 2026-08-02 — архитекторский промах.** Провисела в `pause/`
> с планирования слайса 4, не сверена с новым бэклогом при раздаче T176.
> **T176** («вкладка Files: порт слоя FS и своя узкая вьюха», принята
> `1892d09`, см. `done/T176-files-tab.md` + `report-log/T176-files-tab-
> report.md`) закрыла ровно этот скоуп — read-only дерево `$HOME` в
> `side_panel_right`, порт листинга из Chronos-FM
> (`crates/app/src/side_panel_right/tab/files.rs`), заново, без ссылки на
> T115. Дубль. Уезжает в `rejected/` с этой пометкой, текст ниже —
> исторический контекст плана, не актуален.
