...
**1. Правка `docs/HANDOFF.md` — вне зоны файлов и с подменой факта.**
Зона задачи: `crates/services/src/hermes_acp/{client.rs,session.rs}` и
`composer.rs`. `docs/HANDOFF.md` в неё не входит и вообще принадлежит архитектору —
...
(`git checkout -- docs/HANDOFF.md`); фактическую версию — с замером и с тем, что
