...
### Северная звезда (2026-08-03)

Цель, заявленная автором: чтобы ChronOS производил на людей то же впечатление,
какое когда-то произвёл DankMaterialShell на него — **чтобы человек переходил с
винды на Linux+ChronOS, потому что заманило**.

**Мы играем в другой лиге, и это осознанно.** DMS, end-4/dots и им подобные —
это *конфигурации*: чужие дотфайлы, которые пользователь усыновляет в свою
систему, тянет их обновления, а при поломке чинит в QML и баше. Их сила —
визуальная шлифовка и комьюнити; догонять их по ричингу бессмысленно, там фора в
годы и толпа, которой сам процесс в радость.

ChronOS — **бинарь**: один процесс, свои конфиги, ставится и сносится пакетом.
Отсюда единственная ось, на которой мы можем выиграть и на которой они
структурно не могут:

> **Это продукт, который ставят, а не рис, который усыновляют.**

Переселенец с винды не хочет поддерживать чужой репозиторий. Ему нужно, чтобы
поставилось и работало, а настройки **кликались**, а не редактировались в QML.

Отсюда три следствия, обязательные к учёту в любой задаче:

1. **Вход в воронку — часть продукта.** Пока нельзя поставить одной командой,
   «заманило» некуда вести. На 2026-08-03 у шелла **нет** PKGBUILD/пакета —
   это дыра №1, а не «упаковка потом».
2. **Первый час важнее сотого.** Человек с винды не знает биндов Hyprland.
   Открытие вслепую = удаление. Вкладка Hyprland binds (T193) — актив
   онбординга, а не справочник для своих.
3. **Из коробки должно работать всё бытовое** — звук, яркость, сеть,
   bluetooth, уведомления, питание. Виндузятник не собирает систему по частям;
   любая дырка здесь читается как «Linux не готов», а не «ChronOS не готов».

### Кто судит (2026-08-03)

Не автор и не ассистент — **масса линуксоидов**: r/unixporn, r/linux, GitHub,
Pikabu. Это отдельная ось приёмки, и она жёстче внутренней:

- **Кадр решает.** На r/unixporn соревнуются скриншотами. Дефолтный вид обязан
  выигрывать без «сначала настрой» — иначе продукта не существует.
- **Трение установки — часть продукта.** Поставил/снёс одной командой. Сборка с
  плясками убивает интерес раньше, чем человек увидит UI.
- **«Зачем это вместо Waybar / Hyprpanel / Noctalia / quickshell?»** — вопрос из
  первого комментария. Отвечать должен сам продукт, а не README.
- **Хакабельность.** Конфиги текстом (`bar.toml`, `theme.toml`, плагины Luau),
  не чёрный ящик: рицер, который не может залезть руками, уходит.
- **Доверие.** Отсутствие сети и телеметрии — аргумент, а не мелочь (сетевой
  guard T180 подавать как фичу).
